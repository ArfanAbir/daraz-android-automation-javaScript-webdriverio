...
describe('Daraz Mobile Android ', function () {
    ...
    it('Should Search The Avita Laptop from the Category section in the App', async function () {
        this.timeout(300 * 3000);
            ...
            filter.click();
            await new Promise(res => setTimeout(res, 5000));
            const avita = await driver.elementByXPath("//android.widget.TextView[@text='Avita']");
            expect(avita).to.exist;
            avita.click();
            await new Promise(res => setTimeout(res, 3000));
